...
A Hebrew (Jewish) calendar datepicker that behaves like a native date input. Binds a `Date` through `ng-model` and displays the date in Hebrew.

```<bmml-heb-datepicker ng-model="$data.due_date"></bmml-heb-datepicker>```

See **[bmml-heb-datepicker.md](bmml-heb-datepicker.md)** for the full reference (all settings, theming, and styling overrides).
